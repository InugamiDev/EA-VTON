...
Two entry points:
- run_inference(person_img, garment_img, garment_type) — called by app.py /infer
- run_local_composite(person_path, ...) — legacy async path-based API
"""
...
# intent: synchronous PIL-based entry point for app.py /infer endpoint
# status: done
# next: upgrade torso detection to use pose landmarks when available
# confidence: high
def run_inference(
    ...
    """Run local composite try-on from PIL images.

    Called by app.py _run_inference() for the 'local_composite' backend.
    ...
    # ── Detect torso region ──────────────────────────────────────────
    torso = _detect_torso_region_simple(person)
    ...
    # Fit garment to torso with slight overflow for natural look
    fit_w = int(torso_w * 1.05)
    fit_h = int(fit_w / max(garment_aspect, 0.01))
    ...
    # ── Position garment centered on torso ────────────────────────────
    paste_x = torso["left"] + (torso_w - fit_w) // 2
    paste_y = torso["top"] + (torso_h - fit_h) // 4  # slightly above center
    ...
    # Garment layer with feathered alpha
    garment_layer = Image.new("RGBA", (pw, ph), (0, 0, 0, 0))
    r, g, b, a = garment_resized.split()

    # Feather edges
    feathered_alpha = a.filter(ImageFilter.GaussianBlur(radius=1.5))

    # 85% opacity blending
    alpha_arr = np.array(feathered_alpha, dtype=np.float32)
    alpha_arr = alpha_arr * 0.85
    feathered_alpha = Image.fromarray(alpha_arr.astype(np.uint8))

    garment_final = Image.merge("RGBA", (r, g, b, feathered_alpha))
    ...
    metadata = {
        ...
        "confidence": 0.45,
        "torso_region": torso,
        "garment_size": (fit_w, fit_h),
    }

    return result_rgb, metadata


def _detect_torso_region_simple(person: Image.Image) -> dict:
    """Detect approximate torso region using variance-based heuristics.

    Avoids heavy dependencies (no skimage). Uses image proportions and
    simple luminance variance to estimate shoulder width.
    """
    arr = np.array(person.convert("RGB"), dtype=np.float32)
    h, w, _ = arr.shape

    # Approximate torso top/bottom from image proportions
    # Head ~ top 20%, torso ~ 20-65%, legs ~ 65%+
    torso_top = int(h * 0.20)
    torso_bottom = int(h * 0.68)

    # Estimate shoulder width from luminance variance in a thin horizontal band
    band_top = torso_top
    band_bottom = min(torso_top + max(int(h * 0.08), 10), h)
    torso_band = arr[band_top:band_bottom, :, :]

    if torso_band.shape[0] > 0:
        gray_band = np.mean(torso_band, axis=2)  # H x W luminance
