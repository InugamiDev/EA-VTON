...
_MODEL_DIR = Path(os.environ.get("MODEL_DIR", str(Path(__file__).parent.parent / "models")))
...
def _fallback_estimation(
    ...
    Uses the trained Ridge regression model from BodyM dataset (R²=0.84, MAE=2.1cm)
    if available, otherwise falls back to proportional formulas.
    """
    # Try trained BodyM regression model (sklearn Pipeline with built-in scaler)
    try:
        import sys
        research_root = str(Path(__file__).resolve().parent.parent.parent.parent)
        if research_root not in sys.path:
            sys.path.insert(0, research_root)
        from research.models.train_body_regression import predict_measurements
        preds = predict_measurements(height_cm, weight_kg, gender)

        result.method = "bodym_regression"
        result.chest_circumference = preds["chest_girth_cm"]
        result.waist_circumference = preds["waist_girth_cm"]
        result.hip_circumference = preds["hip_girth_cm"]
        result.shoulder_width = preds["shoulder_breadth_cm"]
        result.arm_length = preds["arm_length_cm"]
        result.leg_length = preds["leg_length_cm"]
        result.torso_length = round(height_cm * 0.30, 1)
        logger.info("Fallback: used trained BodyM regression (R²=0.84)")
        return result
    except Exception as e:
        logger.warning("BodyM regression unavailable, using formulas: %s", e)

    # Hardcoded proportional fallback
